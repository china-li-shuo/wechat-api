...
class Teacher extends BaseController
{
    ...
    public function getClassStatus()
    {
        ...
        $validate->goCheck();
        $class_id = $validate->getDataByRule(input('post.'));
        //接收参数
        ...
        if (!empty($stage) && !empty($group)) {
            ...
            $allStudentInfo = $this->getAdminInfo($uid, $stage, $group, $sort, $class_id['class_id']);
        ...
        $allStudentInfo               = $this->getAdminInfo($uid, $stageID, $firstGroupID, $sort,$class_id['class_id']);
        $allStudentInfo['group_name'] =& $groupName['group_name'];
        if (empty($allStudentInfo)) {
            throw new MissException([
                'msg'       => '此班级下,此阶段下,此分组下,学员信息查询失败',
     ...
     * @throws \think\exception\DbException
     */
    private function getAdminInfo($uid, $stageID, $firstGroupID, $sort,$class_id)
    {
        $classInfo = UserClass::getClassData($uid,$class_id);//此老师所在班级信息
        ...
        foreach ($allStudentInfo as $key => $val) {
            ...
            $allStudentInfo[$key]['avatar_url'] = !empty($userInfo['avatar_url'])?$userInfo['avatar_url']:config('setting.avatar_path');
        }
